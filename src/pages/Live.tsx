...
const Live = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('live-page-streams').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'live_streams'
    }, () => {
    ...
    const podcastChannel = supabase.channel('podcast-live-stream').on('postgres_changes', {
      event: '*',
      schema: 'public',
  ...
  const fetchTVSchedules = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("tv_schedules").select("*").order("created_at", {
        ascending: true
      });
      if (error) throw error;
  ...
  const fetchBlogPosts = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("blog_posts").select("id, title, slug, excerpt, featured_image_url, published_at").eq("published", true).order("published_at", {
        ascending: false
      }).limit(3);
      if (error) throw error;
  ...
  const fetchPodcastStream = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("podcast_live_stream").select("*").eq("is_live", true).single();
  ...
  const getStreamByPage = (pageName: string) => {
    return liveStreams.find(stream => stream.assigned_pages.includes(pageName) && stream.status === 'live');
  };
  const channels = [{
    name: "MetsXMFanZone",
    page: "metsxmfanzone",
      ...
      <main className="pt-16 sm:pt-20">
        <section className="sm:py-12 md:py-16 bg-gradient-to-b from-secondary/20 to-background py-px">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            ...
            {loading ? <div className="text-center py-8 sm:py-12">Loading TV guide...</div> : <div className="space-y-8 mb-12">
                {/* Podcast Live Stream */}
                {podcastStream && podcastStream.vdo_ninja_url && <div>
                ...
                <div>
                  ...
                  return <TVGuideChannel key={channel.page} channelName={channel.name} channelLogo={channel.logo} isLive={!!liveStream} currentShow={liveStream?.title} onWatch={() => navigate(channel.route)} />;
                })}
                  </div>
                </div>
                ...
                {blogPosts.length > 0 && <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {blogPosts.map(post => <Card key={post.id} className="border-2 border-primary/20 hover:border-primary transition-all cursor-pointer" onClick={() => navigate(`/blog/${post.slug}`)}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <TVScheduleCard network="ESPN Network" schedules={tvSchedules.filter(s => s.network === "ESPN Network")} />
                    <TVScheduleCard network="MLB Network" schedules={tvSchedules.filter(s => s.network === "MLB Network")} />
                  </div>
                </div>
              </div>}
