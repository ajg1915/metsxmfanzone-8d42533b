...
const MetsGamecast = () => {
  ...
  useEffect(() => {
    const fetchGameData = async () => {
      try {
        ...
        // Fetch Mets game
        const metsResponse = await fetch(
          `https://statsapi.mlb.com/api/v1/schedule?sportId=1&teamId=121&date=${today}&hydrate=linescore,team,venue`
        );
        ...
        if (allGamesData.dates?.[0]?.games) {
          setOtherGames(allGamesData.dates[0].games.filter((g: any) => 
            g.teams.away.team.id !== 121 && g.teams.home.team.id !== 121
          ));
        }
  ...
  const getSampleGameData = (): GameData => ({
    ...
    linescore: {
      ...
      inningState: 'Top',
      innings: [],
      offense: { first: false, second: false, third: false },
      balls: 0,
      strikes: 0,
      outs: 0
    },
  ...
  const renderBaseDiamond = () => {
    ...
    return (
      <div className="relative w-12 h-12">
        {/* Second base */}
        <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-3 h-3 rotate-45 border-2 ${
          offense?.second ? 'bg-yellow-400 border-yellow-500' : 'bg-muted border-muted-foreground/30'
        }`} />
        {/* Third base */}
        <div className={`absolute top-1/2 left-0 -translate-y-1/2 w-3 h-3 rotate-45 border-2 ${
          offense?.third ? 'bg-yellow-400 border-yellow-500' : 'bg-muted border-muted-foreground/30'
        }`} />
        {/* First base */}
        <div className={`absolute top-1/2 right-0 -translate-y-1/2 w-3 h-3 rotate-45 border-2 ${
          offense?.first ? 'bg-yellow-400 border-yellow-500' : 'bg-muted border-muted-foreground/30'
        }`} />
      </div>
    );
  };
    ...
    return (
      <div className="flex items-center gap-2 text-sm">
        ...
        <div className="flex gap-1">
          {[0, 1, 2, 3].map(i => (
            <div key={i} className={`w-2 h-2 rounded-full ${i < balls ? 'bg-green-500' : 'bg-muted'}`} />
          ))}
        </div>
        <span className="text-muted-foreground ml-2">S</span>
        <div className="flex gap-1">
          {[0, 1, 2].map(i => (
            <div key={i} className={`w-2 h-2 rounded-full ${i < strikes ? 'bg-red-500' : 'bg-muted'}`} />
          ))}
        </div>
      </div>
    );
  ...
  const renderOuts = () => {
    ...
    return (
      <div className="flex items-center gap-1">
        ...
        {[0, 1, 2].map(i => (
          <div key={i} className={`w-2 h-2 rounded-full ${i < outs ? 'bg-orange-500' : 'bg-muted'}`} />
        ))}
      </div>
    );
  };
          ...
          <ScrollArea className="w-full">
            <div className="flex items-center gap-1 p-2 min-w-max">
              {otherGames.slice(0, 12).map((game) => (
                <div key={game.gamePk} className="flex-shrink-0 bg-muted/30 rounded px-3 py-1 text-xs">
                ...
                <div className="grid grid-cols-[1fr_auto_1fr] gap-4 items-center">
                  ...
                  <div className="flex items-center gap-3">
                    <img 
                      ...
                      alt={gameData?.teams.away.team.name}
                      className="w-16 h-16"
                    />
                    <div>
                    ...
                    <thead>
                      <tr className="bg-muted/50">
                        ...
                        {[1,2,3,4,5,6,7,8,9].map(i => (
                          <th key={i} className="p-2 text-center w-8">{i}</th>
                        ))}
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {batters.slice(0, 9).map((batterId: number, idx: number) => {
              ...
              return (
                <tr key={batterId} className="border-t border-border/50">
